...
\begin{figure}[h]
  ...
  \caption{Visualisation of onion routing. At the source, the message starts with three mutually distinct encryptions. Then, each onion router that the packet passes peels off one encryption so that at the destination all onion routing related encryptions have been removed. \protect\footnotemark}
  \label{fig:tor}
\end{figure}
\footnotetext{"Onion diagram" by English Wikipedia user HANtwister. Licensed under CC BY-SA 3.0 via Commons - \url{https://commons.wikimedia.org/wiki/File:Onion_diagram.svg}}
